 ...
 * This function is responsible for inserting a process into one of the scheduling queues. 
 ...
 */
	register struct proc *ready_process;	/* process to run */
	...
	*/
	rdy_head = get_cpulocal_var(run_q_head); // get_cpulocal_var() = get CPU local variable | run_q_head = run queue head
	for (q = 0; q < NR_SCHED_QUEUES; q++) {	 // NR_SCHED_QUEUES = Number of Scheduling Queues
		if(rdy_head[q]) {
			ready_process = rdy_head[q];
			while(rp != NULL)
			{
				if (proc_is_runnable(rp)) {
					total_tickets += ready_process->p_tickets;
				}
				ready_process = rp->p_nextready;
			}
		}
	}
